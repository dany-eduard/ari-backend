  ...
  for (const publisherReport of publisherReports) {
    ...
    if (existingPerson) {
      ...
      if (existsPublisherReport) {
        await publisherReportService.update(existsPublisherReport.id, publisherReport as PublisherReportDto);
        console.log(`ℹ️ El reporte para ${existingPerson.id} ${personName} ya existe. Se actualizó.`);
        continue;
      }
      await publisherReportService.create(publisherReport as PublisherReportDto);
      console.log(`✅ Reporte creado para ${existingPerson.id} ${personName}`);
    } else {
